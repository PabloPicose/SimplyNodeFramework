...
uint16_t resolvePort()
{
    ...
    std::cerr << "Server warning: invalid SNF_HTTPAPP_PORT='" << envPort << "', using " << defaultPort << "\n";
    return defaultPort;
}
...
int main(int argc, char** argv)
{
    ...
    server.on_error.connect([](const std::string& errorMsg) { std::cerr << "Server error: " << errorMsg << "\n"; });
